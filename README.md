...
Частные, loopback, link-local, multicast, неопределенные и CGNAT-адреса из DNS не добавляются в маршруты. Явно заданные исходные IP/CIDR обрабатываются как прежде.
...
[v0.3: изменения и переход с v0.2](docs/releases/v0.3.md). Версионный образ: `ghcr.io/dexogen/iplist-go:v0.3`.

Для выпуска добавьте `docs/releases/<tag>.md`, затем отправьте тег формата `vX.Y` или `vX.Y.Z`. Workflow `Build image` проверит описание, выполнит тесты и проверку запуска контейнера, опубликует образы amd64/arm64 и создаст GitHub Release с digest образа. Уже опубликованные версии не переиспользуются. Обновления списков выпускаются отдельно в sidecar и нового релиза приложения не требуют.
